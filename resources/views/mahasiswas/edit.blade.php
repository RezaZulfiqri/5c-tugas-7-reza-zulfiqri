...
<div class="container pt-4 bg-white">
    <div class="row">
        ...
        <div class="mb-3">
            <label for="ukm_id" class="form-label">Pilih UKM yang Diikuti</label>
            <div class="form-group">
                @foreach ($ukms as $Ukm)
                <div class="form-check form-check-inline">
                    <input type="checkbox" class="form-check-input" name="ukm_id[]" id="{{ $ukm->id }}" value="{{ $ukf->id }}" {{ $mahasiswa->ukfs()->find($ukf->id) ? 'checked' : '' }}>
                    <label for="{{ $ukm->id }}" class="form-check-label">{{ $ukm->nama_ukm }}</label>
                </div>
